...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    ...
    toys toys1
    JOIN
    toys toys2 on toys1.price = toys2.price
    WHERE
    toys1.name != 'Shiny Mouse' AND toys2.name = 'Shiny Mouse'
...
def just_like_orange
  ...
  execute(<<-SQL)
   ...
   cats cats1
   JOIN
   cats cats2 ON cats1.breed = cats2.breed
   WHERE
   cats1.name != 'Orange' AND cats2.name = 'Orange'
   ORDER BY
   cats1.name ASC
...
def toys_that_jet_owns
  ...
  # cats ON cattoys.cat_id = cats.id
  # JOIN
  # toys AS toys2 where toys1.id = toys2.id
  # WHERE
  # cats.name = 'Jet'
  ...
  execute(<<-SQL)
    SELECT
    DISTINCT cats1.name, toys.name

    --ct table = cats1 with toys
    FROM
    cats AS cats1
    JOIN
    cattoys AS cattoys1 ON cats1.id = cattoys1.cat_id
    JOIN
    toys ON cattoys1.toy_id = toys.id

    --self join ct table
    JOIN
    cattoys AS cattoys2 ON toys.id = cattoys2.toy_id
    JOIN
    cats AS cats2 ON cattoys2.cat_id = cats2.id 
    WHERE
    cats1.name != 'Jet' AND cats2.name = 'Jet'
    ORDER BY
    cats1.name ASC
